...
const ProfilePage = () => {
  ...
  const fetchProfile = useCallback(async () => {
    // Get profile ID from URL or authenticated user
    const id = profileId || user?._id;

    // Debugging log
    console.log("🔹 Extracted Profile ID:", id);

    if (!id || id === ":id") {
      console.error(
        "❌ Profile ID is missing or invalid! Cannot fetch profile."
      );
      setLoading(false);
      return;
    }
    ...
    try {
      const headers = isAuthenticated
        ? { Authorization: `Bearer ${user.token}` } // Include only for authenticated users
        : {}; // No headers for public users
  ...
  }, [profileId, user, isAuthenticated]);
